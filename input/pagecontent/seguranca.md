...
- **Consentimento explícito** do titular (Art. 11, I): modelado pelo perfil [`ConsentimentoBiometria`](StructureDefinition-ConsentimentoBiometria.html).
...
- O recurso [`CapturaBiometricaFacial`](StructureDefinition-CapturaBiometricaFacial.html) (`Media`) representa o momento da captura; o campo `content.data` deve conter apenas dados criptografados ou uma referência segura.
...
A extension [`ext-liveness-detection`](StructureDefinition-ext-liveness-detection.html) registra:
...
Todos os eventos de autenticação biométrica (sucesso ou falha) devem ser registrados como [`AutenticacaoBiometricaFacial`](StructureDefinition-AutenticacaoBiometricaFacial.html) (`AuditEvent`), incluindo:
